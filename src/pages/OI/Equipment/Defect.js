import { AutoComplete, Button, Form, Input, Modal, Space, Table, TimePicker } from "antd"
...
const Defect = (props) =>{
    ...
    const searchResult = (query) =>{
        const result = defectData.filter(e=>e?.name?.toLowerCase().includes(query?.toLowerCase()));
        return result.map(e=>{return {value: e.id, label: e.name}});
    }
    ...
    return (
        <Space direction="vertical">
            <Button type="primary" size="large" onClick={()=>setOpenModal(true)} style={{float:'right'}}>Nhập thông tin lỗi</Button>
            <Form>
                <Table
                scroll={{
                    x: 200,
            ...
            <Modal title="Thông tin lỗi" open={openModal} onCancel={()=>setOpenModal(false)}>
                <Form
                form={form}
                onFinish={onFinish}
                layout="vertical">

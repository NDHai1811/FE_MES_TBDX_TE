...
const CheckSheet = (props) => {
  ...
  const errorInput = () => {
    messageApi.open({
      type: "error",
      content:
        "Bạn chưa kiểm tra hết danh sách hoặc dữ liệu nhập vào chưa đúng dịnh dạng",
    });
  };
    ...
    if (checkSheetWork.data != undefined)
      for (let i = 0; i < checkSheetWork.data.length; i++) {
        let d = {
          ...
          time:
            checkSheetWork.is_checked == true
              ? dayjs(checkSheetWork.data[i]?.date_time).format("HH:mm:ss")
              : "",
          date:
            checkSheetWork.is_checked == true
              ? dayjs(checkSheetWork.data[i]?.date_time).format("DD/MM/YYYY")
              : "",
    ...
    },
    {
      ...
      onCell: (record, index) => {
        if (mergeValue.has(record.hm_id)) {
          ...
          const rowCount = dataTable.filter(
            (data) => data.hm_id === record.hm_id
          ).length;
      ...
      render: (value, record, index) => {
        return record.type == 1 ? (
          <Input
            inputMode="numeric"
            value={record?.value}
            disabled={checkSheetWork.is_checked === true ? true : false}
            id={"input" + record.id}
            className="textInput"
            placeholder="Nhập thông số đo"
            type="text"
          />
        ) : checkSheetWork.is_checked == true ? (
  ...
  const save = () => {
    ...
    const allChecked = checkboxInputs.length === countChecked;

    const textInputs = Array.from(document.getElementsByClassName("textInput"));
    const checkInput = textInputs.every(
      (input) => input.value !== "" && !isNaN(input.value)
    );

    if (allChecked && checkInput) {
      (async () => {
        const data = dataTable
          .filter((item) => item.type === 1)
          .map((item) => ({
            id: item.id,
            value: document.getElementById("input" + item.id)?.value,
          }));

        const result = await updateCheckSheetLog({
          data,
        ...
        if (result.success) {
          successSave();
          const res = await getCheckSheetOfMachine({
            machine_id: machine?.value,
          });
          setCheckSheetWork(res.success ? res.data : []);
  ...
  return machine ? (
    <Space direction="vertical" style={{ width: "100%" }}>
      ...
      <Button
        hidden={checkSheetWork.is_checked == true ? true : false}
        onClick={() => save()}
        type="primary"
        size="large"

...
const Screen = (props) => {
  ...
  const permissionUI = [];
  const availableUI = authProtectedRoutes.filter(e => e?.path.includes('ui/') && e?.label && ((userProfile?.permission ?? []).includes('*') || userProfile?.permission?.includes(e?.permission))).map(e => ({ ...e, title: e?.label, link: e.path, permission: e.permission }));

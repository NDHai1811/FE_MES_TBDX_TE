...
import { DeleteOutlined, EditOutlined, UploadOutlined, PlusOutlined, FileExcelOutlined, PrinterOutlined, UserSwitchOutlined } from '@ant-design/icons';
import { DatePicker, Col, Row, Card, Table, Tag, Layout, Divider, Button, Form, Select, AutoComplete, Progress, Space, Tree } from 'antd';
import "../style.scss";
import { Pie, Column, Line, G2 } from '@ant-design/charts';
import { getLines, getKpi, getProducts, getCustomers, getAllMachine } from '../../../api/ui/main';
...
const KPI = (props) => {
      ...
      const [params, setParams] = useState({start_date: dayjs().subtract(6, 'days'), end_date: dayjs()});
      ...
      useEffect(() => {
            setTreeData(listLines.map(e => {
                  ...
                  return { ...e, title: e.name, key: e.id, children: child }
            }));
      }, [listLines])
      ...
      return (
            <React.Fragment>
                  <Row gutter={[8, 8]} style={{ padding: '8px', height: '100vh' }}>
                        <Col span={3}>
                                          ...
                                          <Col span={24}>
                                                <Form.Item label="Bắt đầu" name="start_date">
                                                      <Space direction='vertical' style={{ width: '100%' }}>
                                                            <DatePicker allowClear={false} placeholder='Bắt đầu' style={{ width: '100%' }} value={params.start_date} onChange={(value)=>setParams({...params, start_date: value})}/>
                                                            <DatePicker allowClear={false} placeholder='Kết thúc' style={{ width: '100%' }} value={params.end_date} onChange={(value)=>setParams({...params, end_date: value})}/>
                                                      </Space>
                                                </Form.Item>
                                          </Col>
                                    </Row>
                                    <div style={{padding: '10px', textAlign: 'center'}} layout="vertical">
                                          <Button type='primary' style={{ width: '80%' }} onClick={btn_click}>Truy vấn</Button>
                                    </div>
                              </Card>
                        </Col>
                        <Col span={21}>
                              <Row gutter={[8, 8]}>
                                    {/* <Row gutter={[8, 8]}> */}
                                          <Col span={6}>
                                                <Card title="Tỉ lệ hoàn thành kế hoạch" style={{ padding: '0px' }} >
                                    ...
                                    {/* </Row> */}
                                    <Col span={24}>
                                          <Card style={{ height: '100%' }} title="Bảng thông tin chi tiết các chỉ số KPI" extra={

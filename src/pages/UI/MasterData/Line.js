...
const Line = () => {
    ...
    const loadListTable = async (params) => {
        setLoading(true)
        const res = await getLine(params);
        setData(res.map(e=>{
            return {...e, key: e.id}
        }));
        setLoading(false);
    ...
    const onFinish = async (values) => {
        console.log(values);
        if(isEdit){
            const res = await updateLine(values);
            console.log(res);
            if(res){
                form.resetFields();
                setOpenMdl(false);
            ...
            const res = await createLine(values);
            console.log(res);
            if(res){
                form.resetFields();
                setOpenMdl(false);
    ...
    const deleteRecord = async () => {
        if (listCheck.length > 0) {
            const res = await deleteLine(listCheck);
            setListCheck([]);
            loadListTable(params);
        } else {
    ...
    return <>
        ...
        <Row style={{ padding: '8px', height: '90vh' }} gutter={[8, 8]}>
                ...
                <Col span={21}>
                    <Card style={{ height: '100%' }} title="Quản lý công đoạn" extra={
                        <Space>
                            <Upload

...
import { createTestCriteria, deleteTestCriteria, exportTestCriteria, getTestCriteria, updateTestCriteria } from '../../../api';
...
const TestCriteria = () => {
    ...
    const deleteRecord = async () => {
        if (listCheck.length > 0) {
            const res = await deleteTestCriteria(listCheck);
            setListCheck([]);
            loadListTable(params);
        } else {
    ...
    const editRecord = () => {
        ...
        if (listCheck.length !== 1) {
            ...
            form.setFieldsValue({...result, line: result?.line?.name, reference: result?.ref_line?.name,});
            setOpenMdl(true);
        }
    }
    const insertRecord = () => {
    ...
    return <>
        ...
        <Row style={{ padding: '8px', height: '90vh' }} gutter={[8, 8]}>
                <Col span={3}>
                    ...
                    <div className='mb-3'>
                        <Form style={{ margin: '0 15px' }} layout="vertical">
                            ...
                            <Form.Item label="Hạng mục" className='mb-3'>
                ...
                <Col span={21}>
                    <Card style={{ height: '100%' }} title="Quản lý chỉ tiêu kiểm tra" extra={
                        <Space>
                            <Upload

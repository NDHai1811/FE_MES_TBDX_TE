...
import { createJig, deleteJig, exportJig, getJig, updateJig } from '../../../api';

const Jig = () => {
    document.title = "Quản lý JIG"; 
    ...
    const onFinish = async (values) => {
        console.log(values);
        if(isEdit){
            const res = await updateJig(values);
            console.log(res);
            if(res){
                form.resetFields();
                setOpenMdl(false);
            ...
            const res = await createJig(values);
            console.log(res);
            if(res){
                form.resetFields();
                setOpenMdl(false);
    ...
    const deleteRecord = async () => {
        if (listCheck.length > 0) {
            const res = await deleteJig(listCheck);
            setListCheck([]);
            loadListTable(params);
        } else {
    ...
    const exportFile = async () =>{
        ...
        const res = await exportJig(params);
        if(res.success){
    ...
    return <>
        ...
        <Row style={{ padding: '8px', height: '90vh' }} gutter={[8, 8]}>
                ...
                <Col span={21}>
                    <Card style={{ height: '100%' }} title="Quản lý JIG" extra={
                        <Space>
                            <Upload

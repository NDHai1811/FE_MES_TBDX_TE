...
const UIManufacture = (props) => {
    ...
    },
    {
      ...
      render: (value, item, index) => (
        <InputNumber
          min={1}
          onChange={(value) => {
            setValueInputLsx(item.id, item.lo_sx, value);
            setDataLsx((prev) =>
              prev.map((e) => {
                if (e.lo_sx === item.lo_sx) {
                  return { ...e, sl_pallet: value };
                }
                return e;
              })
            );
      ...
      render: (value, item, index) => {
        ...
        if (item) {
          ...
          for (let key = 0; key < item.sl_pallet; key++) {
            ...
            indents.push(
              <div className="mb-2">
                <span>Pallet {key + 1}:</span>
                <InputNumber
                  min={1}
                  onChange={(value) => {
                    setValueInputDetail(item.lo_sx, key, value);
                  }}
                  disabled={item?.pallet?.length}
                  defaultValue={item?.pallet[key]?.so_luong}
                />
              </div>
            );
          }
    ...
    },
    {
      ...
      render: (value, item, index) => (
        <InputNumber
          min={1}
          onChange={(value) => {
            setValueInput(item.id, value);
          }}
          value={
            valueImport.find((record) => record.id === item.id)
              ? valueImport.find((record) => record.id === item.id).sl_thuc_xuat
              : ""
          }
        />
      ),
  ...
  const editRecord = async () => {
    ...
    if (listPalletCheck.length > 1) {
      ...
      form.setFieldsValue({
        ...
        material_id: result.manvl,
        sl_kho_xuat: result.sl_kho_xuat,
      });
      setOpenMdlEdit(true);
    }
  };
  ...
  return (
    <React.Fragment>
      ...
      <Modal
        title="Thông tin số lượng xuất thực tế"
        open={openMdlNhap}
        onCancel={() => setOpenMdlNhap(false)}
        footer={null}
      >
        <Table
          pagination={false}

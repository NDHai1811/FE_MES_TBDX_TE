...
import { Button, Table, Card, Checkbox, message, Modal, Space, InputNumber, Form, Input } from 'antd';
import "../style.scss";
import { getListLsxUseMaterial, getListMaterialLog, getListScenario, updateScenario } from '../../../api';
...
const Kichban = (props) => {
    ...
    const editRecord = async () => {
        ...
        if (listPalletCheck.length > 1) {
            ...
            form.setFieldsValue({ id: listPalletCheck[0], tieu_chuan: result.tieu_chuan, tieu_chuan_max: result.tieu_chuan_max, tieu_chuan_min: result.tieu_chuan_min, tieu_chuan_kiem_soat_tren: result.tieu_chuan_kiem_soat_tren,tieu_chuan_kiem_soat_duoi: result.tieu_chuan_kiem_soat_duoi})
            setOpenMdlEdit(true);
        }
    }
    return (
        <React.Fragment>
            <Card className='mt-3' title="Kịch bản bất thường" extra={
                <Space>
                    <Button type="primary" onClick={editRecord}>Cập nhật</Button>
                </Space>
            }>
            ...
            <Modal title={titleMdlEdit} open={openMdlEdit} onCancel={() => setOpenMdlEdit(false)} footer={null}>
                <Form style={{ margin: '0 15px' }}
                    ...
                    <Form.Item label="Tiêu chuẩn Max" name="tieu_chuan_max" className='mb-3'>
                        <Input placeholder='Nhập tiêu chuẩn Max'></Input>
                    </Form.Item>
                    <Form.Item label="Tiêu chuẩn Min" name="tieu_chuan_min" className='mb-3' >
                        <Input placeholder='Nhập tiêu chuẩn Min'></Input>
                    </Form.Item>
                    <Form.Item label="Tiêu chuẩn kiểm soát trên" name="tieu_chuan_kiem_soat_tren" className='mb-3'>
                        <Input placeholder='Nhập tiêu chuẩn kiểm soát trên'></Input>
                    </Form.Item>
                    <Form.Item label="Tiêu chuẩn kiểm soát dưới" name="tieu_chuan_kiem_soat_duoi" className='mb-3'>

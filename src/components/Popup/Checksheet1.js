import { Form, Modal, Row, Col, Button, Divider, Radio, Space, Input } from 'antd';
...
const Checksheet1 = (props) =>{
    ...
    return (
        <React.Fragment>
            <Button disabled={!(checksheet.data ?? []).length > 0} type={(isSubmited) ? "primary" : 'default'} danger={result===0} size='large' className='w-100 text-wrap h-100' onClick={() => {!isSubmited && setOpen(true)}}>
                {text}
            </Button>
            <Modal 
                ...
                >
                    ...
                    <Form.List name={'data'}>
                        ...
                        (checksheet.data ?? []).map((e, index)=>{
                            return (
                            <Row gutter={8} className={index === 0 ? '' : 'mt-2'} key={index}>
                                <Col span={12} style={{paddingInline:4}} className='d-flex justify-content-center flex-wrap align-items-lg-center'>
                                    <div className='d-flex justify-content-center align-content-center flex-grow-1 align-items-lg-center p-2' style={{backgroundColor:'#EBEBEB', height:'100%'}}>{e.hang_muc}</div>
                                </Col>
                                <Col span={12}>
                                    <Form.Item name={[index,"id"]} hidden></Form.Item>
                                    <Form.Item name={[index,"result"]} noStyle rules={[{required: true, message: 'Không được để trống'}]}>
                                        <Radio.Group 
